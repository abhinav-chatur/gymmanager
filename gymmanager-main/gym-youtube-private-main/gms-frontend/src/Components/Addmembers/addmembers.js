...
const Addmembers = () => {
  ...
  const uploadImage = async (event) => {
    ...
    data.append('upload_preset', 'gym-management');
      data.append("cloud_name", "dce834buz"); 

    try {
      const response = await axios.post("https://api.cloudinary.com/v1_1/dce834buz/image/upload", data);
  ...
  const fetchMembership = async()=>{
    await axios.get(`${process.env.REACT_APP_API_URL}/plans/get-membership`,{withCredentials:true}).then((response)=>{
  ...
  const handleRegisterButton=async()=>{
    await axios.post(`${process.env.REACT_APP_API_URL}/members/register-member`,inputField,{withCredentials:true}).then((res)=>{
