...
const Member = () => {
    ...
    const fetchData = async (skip, limits) => {
        ...
        await axios.get(`${process.env.REACT_APP_API_URL}/all-member?skip=${skip}&limit=${limits}`, { withCredentials: true }).then((response) => {
    ...
    const handleSearchData = async () => {
        if (search !== "") {
            ...
            await axios.get(`${process.env.REACT_APP_API_URL}/members/searched-members?searchTerm=${search}`, { withCredentials: true }).then((response) => {
                console.log(response)
                setData(response.data.members)
